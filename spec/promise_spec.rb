...
describe Promise do
  ...
  context "chain" do 
    ...
    # it "then is chainable" do 
    #   p = Promise.new
    #   result = []
    #   success = proc { |val| result.push(val); val}
    #   second = proc { |val| result.push(val * 3)}
    #   x = p.then(success).then(second)
    #   p.fulfill(100)
    #   expect(result.length).to eql(2)
    #   expect(result).to eql([100,300])  
    # end

    it "errorback when exception is raised" do 
      p = Promise.new
      result = []
      success = proc { |val| raise "exception" }
      errorback = proc { |reason| result.push(reason) }
      p.then(success).then(nil, errorback)
      p.fulfill(10)
      expect(result.length).to eql(1)
